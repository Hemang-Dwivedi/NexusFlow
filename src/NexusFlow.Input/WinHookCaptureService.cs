...
public interface IWinHookCaptureService
{
	...
	/// <summary>
	/// Forces the P0 reference point to (x, y) for the next remote-routing session.
	/// Must be called from code running on the hook thread (e.g. inside a MouseMove
	/// subscriber) so it takes effect before the current callback finishes updating
	/// _lastX/_lastY with the trigger position.
	/// </summary>
...
public sealed class WinHookCaptureService : IWinHookCaptureService, IDisposable
{
	...
	// Set by SetP0() — prevents the local-routing path from overwriting _lastX/_lastY
	// with the trigger position after SnapCursorToEdge has already set it to the snap position.
	private bool _p0IsLocked;
	...
	{
		// Called from the hook thread (inside a MouseMove subscriber).
		// Sets the P0 reference and prevents the local-path _lastX = x update
		// from overwriting it when the current callback finishes.
		_lastX = x;
		_lastY = y;
		_hasLast = true;
		_p0IsLocked = true;
		...
		try
		{
			...
			if (msg == MouseMessage.WM_MOUSEMOVE)
			{
				...
				if (routeToRemote)
				{
					...
					//
					// P0 is set to the snap position by TargetSwitchingEngine calling
					// SetP0(snappedX, snappedY) from within the trigger event's MouseMove
					// subscriber chain — before this local path runs.  The _p0IsLocked flag
					// prevents the local path below from overwriting that value with the
					// (incorrect) trigger position.
					//
					// If we updated _lastX/_lastY with pt[n-1] here we would get:
					//   dx = pt[n] - pt[n-1] = accel(d_n) - accel(d_{n-1})  [wrong]
					// At constant speed this yields dx=0 → remote cursor stops after frame 1.
				...
				// ── Local routing ─────────────────────────────────────────────────────
				// If SetP0 was called during this event's MouseMove subscriber chain
				// (i.e. SnapCursorToEdge just ran), keep that value — don't overwrite
				// it with the trigger position.
				if (_p0IsLocked)
					_p0IsLocked = false;
				else
